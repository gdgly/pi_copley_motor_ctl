...
/*	int AdcPort,MotorPort,FrocePort;
	...
	pthread_create(&tid6,NULL,(void*)thread_test_press,NULL);
...
void thread_force_port(void)
{
  ...
  int rc = zmq_bind (publisher, "tcp://*:8034");
  assert (rc == 0);
	...
	while(1){
		...
		sprintf(rawdata,"force=%d",force_t);
...
/*					uint32_t init_mark;
					...
					while(1){
						...
						if((pot_temp>0.1)&&(pot_temp<3.0)&&((time_now - init_mark) < 60000)){
